document.addEventListener('DOMContentLoaded', () => {
    ...
    const setupHamburgerMenu = () => {
        ...
        mobileMenu.querySelectorAll('a').forEach(link => {
            link.addEventListener('click', (e) => { // Keep event object
                ...
                if (targetElement) {
                    ...
                    const headerOffset = header ? parseFloat(getComputedStyle(header).height) : 60; // Use mobile height default
                    ...
                    setTimeout(() => {
                        window.scrollTo({
                            ...
                            behavior: 'smooth' // Can use smooth scroll here too
                        });
                    }, 100); // Small delay
                }
            });
    ...
    const setupSmoothScrolling = () => {
        // Only target links in desktop nav
        document.querySelectorAll('.desktop-nav a[href^="#"]').forEach(anchor => {
            anchor.addEventListener('click', function (e) {
                 e.preventDefault(); // Prevent default for desktop nav links
    ...
    setupHamburgerMenu();
    setupSmoothScrolling(); // Now only affects desktop nav automatically
